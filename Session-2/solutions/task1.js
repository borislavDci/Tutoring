...
let brothersCut = Math.floor(liliAge / 2);
let bitcoinsFromToys = toys * toysPrice;
let totalSavedMoney = bitcoins + bitcoinsFromToys - brothersCut;
if (totalSavedMoney >= washingMachinePrice) {
    console.log(`Yes! ${totalSavedMoney - washingMachinePrice} is the remaining bitcoins`);
}
else {
    console.log(`No! ${washingMachinePrice - totalSavedMoney} is not enough `);
}